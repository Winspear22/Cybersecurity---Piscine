...
UrlHelper::UrlHelper(const UrlHelper &src)
{
    *this = src;
}
...
std::string UrlHelper::_resolve_url(const std::string &base_url,
                                    const std::string &link_url)
{
    ...
    // Si on ne trouve pas le protocole, on suppose que c'est du http implicite
    // ou qu'on ne peut rien faire
    if (protocolPos == std::string::npos) return link_url;  // Cas fallback
...
bool UrlHelper::_is_valid_extension(const std::string &url)
{
    ...
    if (pos == std::string::npos) return (FAILURE);
    ...
    {
        if (extension == validExtensions[i]) return (SUCCESS);
        i++;
    }
    return (FAILURE);
}