...
Spider::Spider()
{
    ...
    std::cout << BOLD_GREEN << "Spider constructor called" << RESET
              << std::endl;
}
...
void Spider::addArgumentsToVector(char **argv)
{
    ...
    std::cout << BOLD_GREEN << "Spider parse_arrguments called" << RESET
              << std::endl;
    std::cout << "URL cible définie : " << BOLD_YELLOW << this->_start_url
              << RESET << std::endl;
}
...
bool Spider::argsParser(void)
{
    ...
    while (it != this->_args.end())
    {
        ...
        else if (*it == "-l")
        {
            ...
            if (it + 1 != this->_args.end())
            {
                ...
                {
                    std::string msg = "Error : invalid depth value (" +
                                      std::string(e.what()) + ")";
    ...
    if (this->_start_url.empty())
        return print_error(
            "Error : No URL provided. Usage: ./spider [-rlp] URL");
...
{
    std::cout << BOLD_GREEN
              << "🕷️ Démarrage du Spider sur : " << this->_start_url
              << RESET << std::endl;
    ...
    while (!this->_url_queue.empty())
    {
        ...
        this->_url_queue.erase(
            this->_url_queue.begin());  // On l'enlève de la liste
        ...
        // E. Analyse des liens (Récursivité) -> Ajoute les nouveaux liens dans
        // la queue
            ...
            for (size_t i = 0; i < new_links.size(); i++)
            {
                ...
                if (this->_visited_urls.find(link) == this->_visited_urls.end())
                {
                    if (this->_is_safe_to_scan(link) ==
                        SUCCESS)  // On réutilise is_safe pour vérifier
                                  // domaine/doublons
                    {
                        this->_visited_urls.insert(link);
                        this->_url_queue.push_back(
                            std::make_pair(link, current_depth + 1));
                        std::cout << "  [BFS] Ajout lien (Prof "
                                  << current_depth + 1 << ") : " << link
                                  << std::endl;
                    }
                }
            }
    ...
    std::cout << BOLD_CYAN << "\n📦 Exploration terminée ! "
              << this->_image_urls.size() << " images uniques trouvées."
              << RESET << std::endl;
    ...
    while (it != this->_image_urls.end())
    {
        ...
        if (!image_data.empty())
            this->_saver.save_file(image_data, image_url,
                                   this->_output_dir_path);
    ...
    if (this->_image_urls.empty() && this->_invalid_images_count > 0)
        std::cerr << BOLD_RED << "Error, TOUTES les images ("
                  << this->_invalid_images_count << ") étaient invalides."
                  << RESET << std::endl;
    else
    {
        ...
        if (this->_invalid_images_count > 0)
            std::cout << BOLD_YELLOW << "Succès ! Mais il y'avait "
                      << this->_invalid_images_count << " images rejetées."
                      << RESET << std::endl;
    }
}
...
bool Spider::_is_safe_to_scan(std::string &url)
{
    ...
    if (hash_pos != std::string::npos) url = url.substr(0, hash_pos);
    ...
    while (i < banned_exts.size())
    {
        ...
        {
            return (print_error("[IGNORÉ] Extension interdite (" +
                                banned_exts[i] + ") : " + url));
        }
        i++;
    }
