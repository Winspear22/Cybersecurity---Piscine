...
/*   Updated: 2026/03/03 00:31:08 by adnen            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
private:
  ...
  std::string _readAsciiTag(const std::vector<unsigned char> &data,
                            size_t tiffStart, size_t entryValueOffset,
                            uint32_t count, bool bigEndian);

  /* Parcourt un IFD et affiche les tags connus */
  void _parseIFD(const std::vector<unsigned char> &data, size_t tiffStart,
                 size_t ifdOffset, bool bigEndian, bool isExifSubIFD);
};
