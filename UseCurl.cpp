...
/*   Updated: 2026/02/21 23:15:54 by adnen            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::string UseCurl::getHtml(const std::string &url) {
  ...
  if (curl) {
    CURLcode result; // Variable de type CURLcode qui va stocker le résultat de
                     // la fonction curl_easy_perform
    curl_easy_setopt(curl, CURLOPT_URL,
                     url.c_str()); // Le site où on doit se rendre
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION,
                     _writeCallback); // La fonction que CURL doit utiliser (il
                                      // ne doit pas gérer ça seul)
    curl_easy_setopt(curl, CURLOPT_WRITEDATA,
                     &buffer); // Le buffer dans lequel curl devra mettre tout
                               // ce qu'il télécharge
    curl_easy_setopt(
        curl, CURLOPT_FOLLOWLOCATION,
        1L); // Gestion des redirection (utile pour le http --> https)
    curl_easy_setopt(curl, CURLOPT_TIMEOUT,
                     10L); // Gestion du timeout mis à 10 secondes
    curl_easy_setopt(
        curl, CURLOPT_USERAGENT,
        USER_AGENT); // Curl ne se fait pas passer pour curl, mais pour un
                     // navigateur (certains sites refusent curl)
...
size_t UseCurl::_writeCallback(void *contents, size_t size, size_t nmemb,
                               void *userp) {
  size_t realsize;
  std::string *buffer;
