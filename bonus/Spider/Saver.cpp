...
{
    std::cout << BOLD_GREEN << "Saver copy constructor called" << RESET
              << std::endl;
    *this = src;
}
...
{
    std::cout << BOLD_GREEN << "Saver copy assignment operator called" << RESET
              << std::endl;
    if (this != &src)
    {
    }
                      ...
                      const std::string &output_dir)
{
    ...
    std::string cleanName =
        this->_extract_filename(url);  // ON RECUPERE LE NOM ICI
    ...
    if (!file.is_open())
    {
        std::cerr << BOLD_RED << "Error: could not write to " << fullPath
                  << RESET << std::endl;
        return;
    }
...
std::string Saver::_extract_filename(const std::string &url)
{
    ...
    if (filename.empty() || filename == ".") filename = "default.bin";
